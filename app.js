...
const fs = require('fs');
const DBCParser = require('./utils/DBC.js');
...
app.get("/loadDbcFiles", async(req, res) => {
    await fs.readdir(path.join(__dirname, 'db/dbcFiles'), function(err, files) {
        let fileArray = [];
...
app.get("/downloadDbcFile", (req, res) => {
    const path = './db/dbcFiles/' + req.query.filename;
